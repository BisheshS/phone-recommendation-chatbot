...
function createThumbnailCard(session,url,title,price,rating) {
  if(rating!=null){
    console.log("WHEN THERE IS UNDEFINED RATING-----",title,price,rating);
    return new builder.ThumbnailCard(session)
        ...
        console.log("-------------WHEN THERE IS RATING");
        return new builder.ThumbnailCard(session)
            .title(title)
            .text(price)
            .images([
...
bot.dialog('colorDialog',[
    function(session) {
        ...
        fs.readFile('all_phones_high_to_low.json',(err,data)=>{
          ...
          //array for storing all the names of the high rated phones
          session.conversationData.all_phone_names= {};
          session.conversationData.all_phone_prices= {};
          session.conversationData.all_phone_ratings= {};
          session.conversationData.all_phone_urls= {};
          ...
          for(i=0; i< 3; i++){
            //session,url,title,price,rating
            var card = createThumbnailCard(session,session.conversationData.colored_phone_urls[i],session.conversationData.colored_phone_names[i],session.conversationData.colored_phone_prices[i],session.conversationData.colored_phone_ratings[i]);
            // attach the card to the reply message
            var msg = new builder.Message(session).addAttachment(card);
            session.send(msg);
          }
    ...
    function(session) {
        ...
        var luis_response = getLuisIntent(reply,function(result) {
          ...
          fs.readFile('all_phones_high_to_low.json',(err,data)=>{
            ...
            for(i=0; i< 3; i++){
              //session,url,title,price,rating
              var card = createThumbnailCard(session,session.conversationData.ram_phone_urls[i],session.conversationData.ram_phone_names[i],session.conversationData.ram_phone_prices[i],session.conversationData.ram_phone_ratings[i]);
              // attach the card to the reply message
              var msg = new builder.Message(session).addAttachment(card);
              session.send(msg);
            }            // session.send(output);
    ...
    function(session) {
      ...
      session.conversationData.budget_phone_urls= new Array();
        ...
        var luis_response = getLuisIntent(reply,function(result) {
          ...
          const fs = require('fs');
          //reading from the high rated phones
          fs.readFile('all_phones_high_to_low_2.json',(err,data)=>{
            if(err) throw err;
            var obj = JSON.parse(data);
            var main, name_temp;
            ...
            //
            var jal = 15;
            for (i=0; i < all_phone_prices.length; i++){
              // output = output + "<br/>" + all_phone_names[i];
              // var str = (all_phone_names[i]).toString().toLowerCase();
              // console.log('inside the color_loop -----------------',str);
              if(all_phone_prices[i])
              {
                var a = parseFloat(all_phone_prices[i].replace(/,/g, ''));
              }
              else
              {
                var a = entity_type + 1000000;
              }

              if (a < entity_type){
                jal = jal - 1;
                console.log(all_phone_names[i]+ all_phone_prices[i] + " is below " + entity_type );
            ...
            console.log(output);
            session.send("Displaying top 15 results ")
            session.send(output);
            //creating cards for top3 results
            var kin = 3;
            for(i=0; i< 7; i++){
              if(session.conversationData.budget_phone_ratings[i]){
                //session,url,title,price,rating
                kin = kin - 1;
